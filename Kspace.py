...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.widgets = [self.ui.InputImage1,self.ui.FourierInput1,self.ui.KSpace]
    ...
    def OpenImage(self):
        ...
        for i in range(self.err3.shape[1]):
            self.err3[:,i] = self.err3[:,i]*np.exp(-np.linspace(0,self.Ttotal,self.N)/self.T2star)
    ...
    def KSpace(self):
        if (str(self.ui.comboBox_2.currentText())) == 'Show K-Space Image':
            ...
            msg.setText("Gibb's ringing occurs when the frequencies that are sampled in k-space are not sufficiently high ")
            s = msg.exec_()
            ...
            msg = QMessageBox()
            msg.setWindowTitle("Definition")
            msg.setText("Ghosting happens when even and odd k-space lines are intensity modulated differently")
            s = msg.exec_()
            ...
            msg = QMessageBox()
            msg.setWindowTitle("Definition")
            msg.setText("This particular artefact is due to the fact that when we acquire k-space data, the signal that we base our measurement on decays over time, with a time constant called T2*")
            s = msg.exec_()
