...
def plot_frame3D(vectors, frame, output):
    ...
    # Create 3D axes
    aspect = .952
    figSize = 10  # figure size in inches
    canvasWidth = figSize
...
def animate(vectors, output_file,leapFactor=6, gifWriter='ffmpeg'):
    """
    ...
    if output['output_file']:
        ...
        output['freezeFrames'] = []
        for frame in range(0, vectors.shape[1], leapFactor):
...
def plot_trajectory(file_name, data):
    ...
    x = VECTOR_SIZE* data[0][:, 0]
    y = VECTOR_SIZE* data[0][:, 1]
    z = VECTOR_SIZE* data[0][:, 2]
    ax.plot(x, y, z)
    x = VECTOR_SIZE * data[1][:, 0]
    y = VECTOR_SIZE * data[1][:, 1]
    z = VECTOR_SIZE * data[1][:, 2] + 0.5
    ax.plot(x, y, z)
    x = VECTOR_SIZE * data[2][:, 0]
    y = VECTOR_SIZE * data[2][:, 1]
    z = VECTOR_SIZE * data[2][:, 2] + 1
    ax.plot(x, y, z)
